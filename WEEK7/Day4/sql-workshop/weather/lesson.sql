...
select max(tempf) as max_temp, stations.name as station_name from reports join stations on reports.station_id=stations.id Group By stations.id order by max_temp DESC;
...
select stations.name, reports.time::date as report_date, avg(wind_speed) as avg_wind_speed, count(*) as sample_count
from reports
join stations on reports.station_id=stations.id
group by report_date, stations.name order by report_date, stations.name;
...
select stations.name, reports.time::date as report_date, min(wind_speed) as min_wind_speed, max(wind_speed) as max_wind_speed, count(*) as sample_count
from reports
join stations on reports.station_id=stations.id
group by report_date, stations.name order by report_date, stations.name;
